...
import { AnimalCard, Card, DisasterCard, ElementCard, GameState, PlantCard, PositionedCard } from "@/state/types";
...
export const supplyDeckPositions = (gameState: GameState): [number, number, number][] => {
  ...
  if (gameState.players.length > 0) {
    positions.push([0 - getSupplyDeckOffset(gameState.players[0].hand), playerCardsYStart, 0]);
  }

  if (gameState.players.length > 1) {
    positions.push([upperXBoundary - cardHeight / 2, 0 - getSupplyDeckOffset(gameState.players[1].hand), 0]);
  }

  if (gameState.players.length > 2) {
    positions.push([0 + getSupplyDeckOffset(gameState.players[2].hand), upperYBoundary - cardHeight / 2, 0]);
  }

  if (gameState.players.length > 3) {
    positions.push([lowerXBoundary + cardHeight / 2, 0 + getSupplyDeckOffset(gameState.players[3].hand), 0]);
  }
